...
export default function BillAndPayPage() {
    ...
    const penalty = new Date().getDate() > 20 ? rates.penaltyRate : 0;

    const totalAmount = bill?.totalAmount || 0;

    // Correct service fee computation
    const serviceFee =
        paymentType !== 'Cash'
            ? Math.ceil(totalAmount / 100) * rates.serviceFeeRate
            : 0;

    const totalAmountDue = (totalAmount + serviceFee + penalty).toFixed(2);
                    ...
                    <div className="flex flex-col lg:h-full">
                        ...
                        <div className="mt-2 flex justify-between items-center">
                            <div className="text-xs text-gray-600">
                                Notification Status: {bill.notifStatus ?? "N/A"}
                            </div>
                            <button
